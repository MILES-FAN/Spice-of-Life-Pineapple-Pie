...
public final class FoodList implements FoodCapability {
	...
	public void addFood(Item food, Map<FoodInstance, Integer> foodMap) {
		if (!SOLRingoPieConfig.shouldCount(food) && !SOLRingoPieConfig.shouldForbiddenCount()) {
			return;
		}
		...
		for (Map.Entry<FoodInstance, Integer> entry : foodMap.entrySet()) {
			...
			if (lastEaten >= SOLRingoPieConfig.size()) {
		...
		if (SOLRingoPieConfig.shouldCount(food)) {
	...
	public double simulateFoodAdd(Item food) {
		...
		for (Map.Entry<FoodInstance, Integer> entry : uniqueFoods.entrySet()) {
			...
			else if (lastEaten >= SOLRingoPieConfig.size()) {
				change -= diversityContribution;
			}
			else {
	...
	private static double calculateTimePenalty(int lastEaten) {
		int size = SOLRingoPieConfig.size();
		int startDecay = SOLRingoPieConfig.startDecay();
		int endDecay = SOLRingoPieConfig.endDecay();
		double minContribution = SOLRingoPieConfig.minContribution();
	...
	public static double getComplexity(FoodInstance food) {
		...
		if (foodProperties != null)
		{
			...
			if (average < 5) {
				return average * SOLRingoPieConfig.defaultContribution() / 5f;
			}

			return SOLRingoPieConfig.defaultContribution() * 4 * Math.log10(average - 4) + 1;
		}

		return SOLRingoPieConfig.defaultContribution();
	}
