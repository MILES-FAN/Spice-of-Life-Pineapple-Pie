package com.rizastar.solringopie.integration;
...
public class Origins {
    ...
    public static boolean hasRestrictedDiet(Player player) {
        ...
        if (originContainer == null) {
            SOLRingoPie.LOGGER.warn("No IOriginContainer found for player: " + player.getName().getContents());
            return false;
        }
        ...
        for(ResourceKey<Origin> originKey : originContainer.getOrigins().values()) {
            ...
            if (origin == null) {
                SOLRingoPie.LOGGER.warn("Player " + player.getName().getContents() +
